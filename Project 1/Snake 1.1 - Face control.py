...
def main():
    ...
    lower = np.array([0, 27, 35])
    higher = np.array([255, 255, 255])
    cascade = cv2.CascadeClassifier("D:\\Python\\Lesson 1\\haarcascade_frontalface_alt2.xml")
    face_x = []
    face_y = []
    while True:
        _, frame = cap.read()
        ...
        if len(faces) > 0:
            xmax, ymax, wmax, hmax = faces[0][0], faces[0][1], faces[0][2], faces[0][3]
            ...
            if len(face_x) == 2:
                del face_x[0]
                face_x.append(cx)
                if face_x[1] - face_x[0] >= 10 and not game.snake.dir_x == -1:
                    game.snake.dir_y = 0
                    game.snake.dir_x = 1
                elif face_x[0] - face_x[1] >= 10 and not game.snake.dir_x == 1:
                    game.snake.dir_y = 0
                    game.snake.dir_x = -1
            if len(face_y) == 2:
                del face_y[0]
                face_y.append(cy)
                if face_y[1] - face_y[0] >= 10 and not game.snake.dir_y == -1:
                    game.snake.dir_y = 1
                    game.snake.dir_x = 0
                elif face_y[0] - face_y[1] >= 10 and not game.snake.dir_y == 1:
                    game.snake.dir_y = -1
                    game.snake.dir_x = 0
